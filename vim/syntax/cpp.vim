...
" from other file
" Preprocs
syn keyword cppDefined defined contained containedin=cppDefine
hi def link cppDefined cppDefine

" Functions
syn match cppUserFunction "\<\h\w*\>\(\s\|\n\)*("me=e-1 contains=cppType,cppDelimiter,cppDefine
syn match cppUserFunctionPointer "(\s*\*\s*\h\w*\s*)\(\s\|\n\)*(" contains=cppDelimiter,cppOperator

hi def link cppUserFunction cppFunction
hi def link cppUserFunctionPointer cppFunction
...
" foldmethod=syntax fix, courtesy of Ivan Freitas
hi def link cppBraces Delimiter
